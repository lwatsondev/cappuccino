...
def test_set_user_value(bot):
    bot.ircdb.set_user_value("dbuser", "lastfm", "testaccount")
    assert bot.ircdb.get_user_value("dbuser", "lastfm") == "testaccount"


def test_set_user_value_upsert(bot):
    bot.ircdb.set_user_value("upsertuser", "lastfm", "original")
    bot.ircdb.set_user_value("upsertuser", "lastfm", "updated")
    assert bot.ircdb.get_user_value("upsertuser", "lastfm") == "updated"


def test_del_user_value(bot):
    bot.ircdb.set_user_value("deluser", "lastfm", "todelete")
    bot.ircdb.del_user_value("deluser", "lastfm")
    assert bot.ircdb.get_user_value("deluser", "lastfm") is None


def test_get_user_value_missing(bot):
    assert bot.ircdb.get_user_value("nosuchuser", "lastfm") is None


def test_json_dump_includes_set_values(bot):
    bot.ircdb.set_user_value("jsonuser", "lastfm", "scrobbler")
...
def test_http_server(bot):
    bot.ircdb.set_user_value("serveruser", "lastfm", "scrobbler")
