...
class Rice(Plugin):
    ...
    def _generic_db(self, mask, target, args):  # noqa: C901
        ...
        if args["--add"] or args["-a"]:
            values = self.bot.ircdb.get_user_value(mask.nick, category) or []
            if len(values) + len(args["<values>"]) > max_user_entries:
                ...
                for value in args["<values>"]:
                    values.append(value)
                self.bot.ircdb.set_user_value(mask.nick, category, values)
            ...
            values = self.bot.ircdb.get_user_value(mask.nick, category)
            if not values:
                response = f"You do not have any {category} to remove."
            else:
                indexes = set(args["<ids>"])
                if "*" in indexes:
                    self.bot.ircdb.del_user_value(mask.nick, category)
        ...
        if args["<id>"] is not None:
            try:
                index = _from_user_index(args["<id>"])
                value = self.bot.ircdb.get_user_value(user, category)[index]
            except ValueError, IndexError, TypeError:
                return "Invalid ID."
            value = style(value, reset=True)
            return f"{user_tag} {value}"

        values = self.bot.ircdb.get_user_value(user, category)
        if values:
            indexed_values = []
